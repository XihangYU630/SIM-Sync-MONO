...
def SimSyncReg_mosek9(N, edges, pointclouds, scale_gt=None, Weights = None, reg_lambda=1):
    ...
    with mosek.Env() as env:
        ...
        # Create a Task object within the environment
        with env.Task() as task:
            ...
            solsta = task.getsolsta(mosek.soltype.itr)
            if (solsta == mosek.solsta.optimal):
                # Specify the index of the semidefinite variable you are interested in
                var_index = 0
                size = 3*N
                barx = [0.] * (size * (size + 1) // 2)
                task.getbarxj(mosek.soltype.itr, var_index, barx)
